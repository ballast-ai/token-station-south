//! The official AWS Bedrock Converse provider component: a wit-bindgen shell
//! around the conformance crate's native reference implementation.
//!
//! Gate ② froze its fixture pack against `BedrockConverseReferenceV1`; this
...
//!
//! This component is the only provider guest on the `host_signed` arm: it never
//! sees a credential, and the host's request finalizer signs the request after
//! `build_http_request` returns. Nothing in this shell differs because of that —
//! the arm is declared in `manifest.json` and honoured by the reference
//! implementation leaving `descriptor.auth` unset.
...
struct BedrockConverse;

impl Guest for BedrockConverse {
    fn metadata() -> AdapterMetadata {
        let reported = BedrockConverseReferenceV1.metadata();
        AdapterMetadata {
            name: reported.name,
    ...
    fn model_capabilities(provider_config: String) -> Result<String, String> {
        abi::model_capabilities_json(&BedrockConverseReferenceV1, &provider_config)
    }

    fn build_http_request(chat_request: String, provider_config: String) -> Result<String, String> {
        abi::build_http_request_json(&BedrockConverseReferenceV1, &chat_request, &provider_config)
    }

    fn parse_response(response_parts: String) -> Result<String, String> {
        abi::parse_response_json(&BedrockConverseReferenceV1, &response_parts)
    }

    fn parse_stream_chunk(chunk: Vec<u8>) -> Result<String, String> {
        let mut stream = STREAM.lock().expect("single-threaded guest");
        stream
            .get_or_insert_with(|| abi::StreamAbiV1::new(&BedrockConverseReferenceV1))
            .parse_chunk_json(&chunk)
    }

    fn map_provider_error(response_parts: String) -> Result<String, String> {
        abi::map_provider_error_json(&BedrockConverseReferenceV1, &response_parts)
    }
}

export!(BedrockConverse);
