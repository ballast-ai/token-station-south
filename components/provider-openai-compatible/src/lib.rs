//! The official OpenAI-compatible provider component: a wit-bindgen shell
//! around the conformance crate's native reference implementation.
//!
//! Gate ② froze its fixture pack against `OpenAiCompatibleReferenceV1`; this
//! crate compiles that same implementation to `wasm32-wasip2`, so "the
//! sandboxed output equals the native output" is a property of construction
//! that the sandbox parity test then proves end to end.
