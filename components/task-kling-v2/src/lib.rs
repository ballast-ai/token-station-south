//! The task-v2 Kling guest is a thin shell over the shared native dialect.
...
impl Guest for KlingTask {
    fn metadata() -> AdapterMetadata {
        let metadata = KlingTaskReferenceV2.metadata();
        AdapterMetadata {
            name: metadata.name,
    ...
    fn build_submit_request(
        ...
        abi::build_submit_request_json(&KlingTaskReferenceV2, &config, &request, &minted)
    }
    fn parse_submit_response(parts: String) -> Result<String, String> {
        abi::parse_submit_response_json(&KlingTaskReferenceV2, &parts)
    }
    fn build_observe_request(
        ...
        abi::build_observe_request_json(&KlingTaskReferenceV2, &config, &model, &id, &locator)
    }
    fn parse_observation(parts: String) -> Result<String, String> {
        abi::parse_observation_json(&KlingTaskReferenceV2, &parts)
    }
    fn build_artifact_request(
        ...
        abi::build_artifact_request_json(&KlingTaskReferenceV2, &config, &locator, &observation)
    }
    fn render_success(
        observation: String,
        ...
        abi::render_success_json(&KlingTaskReferenceV2, &observation, fetched.as_deref(), &context)
    }
    fn map_terminal_failure(observation: String) -> Result<String, String> {
        abi::map_terminal_failure_json(&KlingTaskReferenceV2, &observation)
    }
}
export!(KlingTask);
