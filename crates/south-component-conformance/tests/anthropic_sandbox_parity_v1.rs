//! The Anthropic component's S3 acceptance test: the shipped component,
...
fn component_wasm() -> &'static Path {
    ...
    WASM.get_or_init(|| {
        let status = Command::new("bash")
            .arg(repo_root().join("scripts/build-anthropic-component.sh"))
            .status()
            .expect("bash is on PATH");
        assert!(
            status.success(),
            "the Anthropic component must build; run `rustup target add wasm32-wasip2` if the \
             target is missing"
        );
        repo_root().join(
            "components/provider-anthropic/target/wasm32-wasip2/release/provider_anthropic.wasm",
        )
    })
}
...
fn the_shipped_package_passes_gate_one_and_the_tuple_handshake() {
    ...
    assert_eq!(component.metadata(), AnthropicReferenceV1.metadata());
