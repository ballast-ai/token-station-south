//! The Gemini component's S3 acceptance test: the shipped component,
...
fn component_wasm() -> &'static Path {
    ...
    WASM.get_or_init(|| {
        let status = Command::new("bash")
            .arg(repo_root().join("scripts/build-gemini-component.sh"))
            .status()
            .expect("bash is on PATH");
        assert!(
            status.success(),
            "the Gemini component must build; run `rustup target add wasm32-wasip2` if the \
             target is missing"
        );
        repo_root()
            .join("components/provider-gemini/target/wasm32-wasip2/release/provider_gemini.wasm")
    })
}
...
fn shipped_pack() -> FixturePackV1 {
    FixturePackV1::load(&Path::new(env!("CARGO_MANIFEST_DIR")).join("fixtures-gemini"))
        .expect("the shipped fixture pack loads")
}

/// The run that proves "the sandboxed output equals the native output": the
/// suite's expectations were frozen against the native reference, so a pass
/// here is byte-identity per case.
#[test]
...
/// The two components ship distinct identities and distinct packs. A copy of
/// one package under the other's name would pass every check above on its
/// own; only comparing them catches it.
#[test]
fn the_two_official_components_are_not_the_same_package() {
    let (_, anthropic) = shipped_manifest();
    let openai: ComponentManifestV1 = serde_json::from_str(
        &std::fs::read_to_string(
            repo_root().join("components/provider-openai-compatible/manifest.json"),
        )
        .expect("the other manifest reads"),
    )
    .expect("the other manifest parses");

    assert_ne!(anthropic.name, openai.name);
    assert!(
        anthropic.providers.iter().all(|dialect| !openai.providers.contains(dialect)),
        "the two components must not claim the same dialect"
    );
    assert_ne!(
        anthropic.conformance.fixtures, openai.conformance.fixtures,
